...
public class ApiController {
    ...
    /**
     * Query an asset by assetId on Asset chaincode
     *
     * @param assetId The id of the asset to query
     * @return Asset json object
     */
    @RequestMapping(path = "/assets/{assetId}", method = RequestMethod.GET)
    public ResponseEntity<?> queryAsset(@PathVariable(name = "assetId") String assetId) {
        try {
            log.debug("Querying asset by assetId:" + assetId);
            ...
            // query chaincode
            String res = service.queryChaincode(service.getClient(), service.getChannel(),
                    "asset", "queryAsset", "{\"assetId\":\""+assetId+"\"}");

            // convert the response into Car object
            ObjectMapper objectMapper = new ObjectMapper();
            Asset asset = objectMapper.readValue(res, Asset.class);
            asset.setAssetId(assetId);
            return new ResponseEntity<>(asset, HttpStatus.OK);
        } catch (EnrollmentNotFoundException | AppException e){
            log.error("Error while querying - " + e.getMessage());
            return new ResponseEntity<>("Error querying asset - " + e.getMessage(), HttpStatus.BAD_REQUEST);
        } catch (ManagedBlockchainServiceException e) {
            log.error("Error querying asset, ManagedBlockchainService startup failed - " + e.getMessage());
            return new ResponseEntity<>("Error querying asset, ManagedBlockchainService startup failed - "
                    + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        } catch (Exception e) {
            log.error("Error while querying - function:queryAsset chaincode:asset");
            e.printStackTrace();
            return new ResponseEntity<>("Error querying asset, chaincode query failed", HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    /**
     * Inserts a new Asset by invoking the `createAsset` function in asset chaincode
     *
     * @param car json object to insert
     * @return
     */
    @RequestMapping(path = "/assets", method = RequestMethod.POST)
    public ResponseEntity<?> invokeAsset(@RequestBody @Valid Asset asset) {
        try {
            log.debug("Inserting new asset:" + asset.toString());
            ...
            // build arguments list required by the chaincode
            String[] arguments = {asset.toString()};

            // invoke createCar function on fabcar chaincode
            service.invokeChaincode(service.getClient(), service.getChannel(), "asset",
                    "createAsset", arguments);

            return new ResponseEntity<>("Asset created successfully", HttpStatus.ACCEPTED);
        } catch (EnrollmentNotFoundException | AppException e){
            log.error("Error creating asset - " + e.getMessage());
            return new ResponseEntity<>("Error creating asset - " + e.getMessage(), HttpStatus.BAD_REQUEST);
        } catch (ManagedBlockchainServiceException e) {
            log.error("Error creating car, ManagedBlockchainService startup failed - " + e.getMessage());
            return new ResponseEntity<>("Error creating asset, ManagedBlockchainService startup failed - "
                    + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        } catch (Exception e) {
            log.error("Error while invoking - function:createAsset chaincode:asset");
            e.printStackTrace();
            return new ResponseEntity<>("Error creating asset, chaincode invocation failed", HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
